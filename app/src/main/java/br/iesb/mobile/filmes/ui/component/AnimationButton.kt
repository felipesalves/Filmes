package br.iesb.mobile.filmes.ui.component
...
import br.iesb.mobile.filmes.util.dp
import br.iesb.mobile.filmes.util.getBitmap
...
//            canvas.drawCircle(x, y, radius, it)
    ...
    fun setUIState(uiState: UIState, isAnim: Boolean) {
        ...
        if (isAnim) {
            runAnimation().apply {
                doOnEnd {
                    this@AnimationButton.uiState = uiState
                }
            }
        } else {
    ...
    private fun runAnimation(): ValueAnimator {
        return ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            doOnStart {
                this@AnimationButton.uiState = UIState.Animating
            }
            duration = 500L
            start()
        }
    ...
    private fun lerp(a: Float, b: Float, c: Float, fraction: Float): Float {
        return if (fraction <= 0.5f) {
            MathUtils.lerp(a, b, fraction * 2)
        } else {
            val tempFraction = fraction - 0.5f
            MathUtils.lerp(b, c, tempFraction * 2)
        }
    }
